---
description: Simplify code with project-aware context, explicit scope, and behavior-preserving validation
model: glm-5
thinking: medium
---
...
Supported flags:
- `--from <path>` optional seed notes / hotspot report
- `--scope <path-or-glob>` optional path constraint to emphasize target files or directories
- `--fast` (default)
- `--thorough`
- `--hotspots-only` focus on the most meaningful complexity hotspots only
- `--max-function-lines <n>` optional simplification heuristic
...
- **Goal mode**: otherwise, treat `TARGET` as freeform simplify goal text

Set:
- `MODE = ticket|goal`
- `RUN_ID = <ticket-id>` in ticket mode, otherwise kebab-case of `TARGET`
- `CHAIN_DIR = .subagent-runs/tf-simplify/<RUN_ID>`
- `SEED_FILE = <CHAIN_DIR>/simplify-seed.json`
- `TARGET_TOPIC_DIR = .tf/knowledge/topics/<RUN_ID>`

Ensure directories exist:
- `.subagent-runs/tf-simplify`
- `<CHAIN_DIR>`
- `.tf/knowledge/topics`
- `<TARGET_TOPIC_DIR>`
...
```json
{
  ...
  "hotspots_only": true,
  "max_function_lines": "<MAX_FUNCTION_LINES_OR_EMPTY>",
  "speed_profile": "fast|thorough",
  "summary": "One-line description of the simplification objective"
}
```
...
3. Set `SEED_READS = ["simplify-seed.json", "from-seed.md"]`.

Otherwise:
- `SEED_READS = ["simplify-seed.json"]`
...
## 4) Phase 1 (always sync): Build Anchored Simplification Context
  ...
  "chain": [
    {
      "parallel": [
        {
          "agent": "scout",
          "task": "Scout codebase context for simplification target '<TARGET>'. Read simplify-seed.json FIRST. Focus on complexity hotspots, affected files, relevant tests, and control-flow risk areas. Respect --scope when present.",
          "reads": <SEED_READS>,
          "output": "scout-context.md"
        },
        {
          "agent": "context-builder",
          "task": "Build anchored simplification context for '<TARGET>'. Read simplify-seed.json FIRST. Read PROJECT.md when present, use AGENTS.md for repo operating guidance, include lessons from .tf/AGENTS.md plus relevant .tf/knowledge, and emphasize behavior-preservation constraints plus complexity-reduction goals. If from-seed.md exists, synthesize it.",
          "reads": <SEED_READS>,
    ...
    },
    {
      ...
      "task": "Merge scout-context.md and anchor-context-base.md into anchor-context.md. Preserve all anchor sections and append scoped code context plus hotspot findings.",
...
```json
{
  ...
  "maxOutput": { "bytes": 200000, "lines": 5000 },
  "chain": [
    { "agent": "<PLAN_AGENT>", "task": "Create a behavior-preserving simplification plan for '<TARGET>'. Respect PROJECT.md, AGENTS.md, the anchor context, and any --scope / --hotspots-only / --max-function-lines constraints.", "reads": ["anchor-context.md"], "output": "plan.md" },
    { "agent": "simplifier", "task": "Execute the simplification for '<TARGET>' according to plan.md. Preserve behavior, reduce complexity, improve clarity, and constrain changes to the intended scope.", "reads": ["anchor-context.md", "plan.md"], "output": "implementation.md" },
    {
      "parallel": [
        { "agent": "reviewer", "task": "Initial review for simplification target '<TARGET>'. Review only the changes introduced by this simplification. Focus on behavior preservation, readability, regressions, and whether simplification made intent clearer rather than more obscure.", "reads": ["implementation.md", "plan.md", "anchor-context.md"], "output": "review.md" },
        { "agent": "tester", "task": "Run the most relevant tests and checks for simplification target '<TARGET>'. Prioritize changed modules, behavior-sensitive paths, and the highest-signal validations available.", "reads": ["implementation.md", "plan.md", "anchor-context.md"], "output": "test-results.md" }
      ],
      "concurrency": 2,
      "failFast": false
    },
    { "agent": "fixer", "task": "Apply one fix pass for simplification target '<TARGET>'. Prioritize test failures first, then critical/major review findings. Preserve the intended simplification scope.", "reads": ["implementation.md", "review.md", "test-results.md", "plan.md", "anchor-context.md"], "output": "fixes.md" },
    { "agent": "reviewer", "task": "Quick re-check for simplification target '<TARGET>'. Review only the changed files and hunks touched by implementation and fixes. State clearly whether behavior preservation and readability are an unambiguous pass.", "reads": ["implementation.md", "review.md", "test-results.md", "fixes.md", "plan.md", "anchor-context.md"], "output": "review-post-fix.md" },
...
3. Summarize what was simplified and which files were affected.
